...
# MAGIC 
# MAGIC One last thing about the IDF calculation that's worth noting is that is lowers the value for n-grams as their frequency in the overall collection of n-grams increases (hence the *inverse* in inverse document frequency). As such, very common n-grams will receive little consideration in later evaluations but they will still take up space in your feature vector.  If you would like to exclude the most common n-grams, you might consider a funtion to remove n-grams from the dataset based on some relative threshold using something like a SQL Transformer. This isn't implemented here for the sake of simplicity:
# MAGIC 
...
# MAGIC 
# MAGIC We rarely have just the name to go off of for determining a match.  Information such as a product's description can help us refine our comparison of products, distinguising similairly named but otherwise different products.
# MAGIC 
